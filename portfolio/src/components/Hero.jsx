...
const Hero = () => {
  const { darkMode, toggleDarkMode } = useDarkMode();  // global darkMode state ve toggle fonksiyonu
  ...
  const languageText = languageToggle.text;
    ...
    <div className={`hero-container flex overflow-hidden ${darkMode ? 'dark' : ''}`}>
      <div className="hero-blue-section h-[671px] w-[1550px] bg-bl dark:bg-d-bl">
        <div className="language-theme-switch text-[15px] h-[18px] w-[138px] absolute top-[35px] left-[895px] font-semibold">
          <button className="language-toggle">
            <span className="language-text">
              <span className={`${darkMode ? 'text-indigo-200' : 'text-[#CBF281]'}`}>{languageText.split("'")[0]}</span>
              <span className={`${darkMode ? 'text-[#777777]' : 'text-[#D9D9D9]'}`}>{`'${languageText.split("'")[1]}`}</span>
            </span>
          </button>
        </div>
        <div className='hero-name-section w-[960px] h-[72px] absolute top-[37px] left-[280px]'>
          <h2 className="hero-name w-[700px] text-[32px] leading-[72px] text-yellw font-bold">{profile.name}</h2>
        </div>
        <div className='hero-content w-[955px] h-[375.89px] absolute top-[148px] left-[280px] flex justify-between'>
          <div className='hero-content-left pt-24'>
            ...
            <p className="hero-description pt-8 w-[528.88px] h-[58px] font-normal text-[24px] leading-[29.05px] text-white dark:text-gray-300">{profile.description}</p>
      ...
      <div className="green-container h-[671px] w-[710px] left-[1008px] bg-yellw dark:bg-d-bg">
        <div className="theme-switch mx-8 my-8 flex items-center space-x-4">
          <button className="theme-toggle w-[55px] h-[24px] bg-[#8F88FF] dark:bg-[#3A3A3A]  rounded-full flex items-center p-1" onClick={toggleDarkMode}>
            <div className={`theme-indicator w-4 h-4 bg-yellow-300 rounded-full transition-transform duration-300 ease-in-out transform ${darkMode ? 'translate-x-[31px] rotate-[-180deg]' : 'translate-x-0'}`}></div>
          </button>
          <p className="text-[#4731D3] dark:text-[#D9D9D9] font-semibold text-[15px] leading-[18.15px]">{darkMode ? "LIGHT MODE" : darkModeToggle.text}</p>
        </div>
      </div>
    </div>
  );
